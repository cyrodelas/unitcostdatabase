...
`{type}` is restricted to the 46 entries in `application/config/reference_data.php`; arbitrary table access is not allowed. `Auth_cli` is CLI-only and has no web route. `Welcome/index` remains as a compatibility redirect to `/dashboard`.
